...
DashboardView = exports.DashboardView = BaseView.extend({
  ...
  addTab: function(nextTab, tab){
    var isActive, tabModel, tabView, tabId, navEl, graphs, _this = this;
    isActive = !this.subviews.length;
    tabModel = new BaseModel(tab);
    tabView = this.addSubview(new DashboardTabView({
      model: tabModel
    }));
    tabId = tabView.getTabId();
    navEl = $("<li class='tab-button " + tabId + "-button'><a href='#" + tabId + "' data-toggle='tab'>" + tab.name + "</a></li>").appendTo(this.$("nav > ul.nav"));
    if (isActive) {
      navEl.add(tabView.$el).addClass('active');
    }
    graphs = _(tab.graph_ids).map(function(it){
      return _this.model.graphs.get(it);
    });
