var TimeSeriesData, CSVData, limn, op, ReadyEmitter, BaseModel, BaseList, ModelCache, Metric, MetricList, DataSource, DataSourceList, ALL_SOURCES, sourceCache, _ref, _;
_ref = require('timeseries'), TimeSeriesData = _ref.TimeSeriesData, CSVData = _ref.CSVData;
limn = require('../client');
...
DataSource = exports.DataSource = BaseModel.extend({
  ...
  onLoadDataError: function(jqXHR, txtStatus, err){
    console.error(this + " Error loading data! -- " + txtStatus + ": " + (err || ''));
    this.unwait();
...
sourceCache = exports.sourceCache = new ModelCache(DataSource, {
  ready: false,
  cache: ALL_SOURCES
});
limn.on('main', function(){
  return $.getJSON(limn.mount('/datasources/all'), function(data){
    ...
    return DataSource.triggerReady();
  });
});
